...
static void drawLightGraphics(lpLight_t *p)
{
	...
	for (i = 0; i < 3; i++) {
		if (p->color[i] < 0. || p->color[i] > 1.)
			SDL_Log("drawLight: color out of range color[%d] = %f\n", i, p->color[i]);
	}
...
static void sampleData8_error(lpLight_t *p)
{
	...
	if (!flag) {
		SDL_Log("sampleData8: light %s has no data bound to it.\n", p->name);
		flag = true;
	}
#endif
}
...
static void sampleData16(lpLight_t *p)
{
	...
	// 	SDL_Log("sampleData16: clock stepped backward\n");
	...
	while ((n = Parser_parse(&parser, &result)) < 0) {
		if (n != PARSER_DONE) {
			...
			if (!strcmp(light_parse_rules[result->cmd_idx].cmd, "color")) {
				...
				light->obj_refname = (char *) malloc(strlen(result->strings[0]) + 1);
...
int Lpanel_addLightToGroup(Lpanel_t *p, int lightnum, int groupnum)
{
	if (groupnum >= LP_MAX_LIGHT_GROUPS) {
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
			     "error: light %s invalid group number (%d)\n",
	...
	if (p->light_groups[groupnum].num_items + 1 > p->light_groups[groupnum].max_items) {
		...
		new_list = (int *) realloc(p->light_groups[groupnum].list,
					   sizeof(int) * (p->light_groups[groupnum].max_items + 1));
...
bool Lpanel_bindLight8(Lpanel_t *p, const char *name, void *loc, int start_bit_number)
{
	...
	for (i = 0; i < num_names; i++) {
		if (bitnum <= 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindLight8: light %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
			     ...
			     uint8_t mask)
{
	...
	for (i = 0; i < num_names; i++) {
		if (bitnum <= 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindLight8invert: light %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
		...
		if (light) {
			...
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindLight8invert: light %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
...
bool Lpanel_bindLight16(Lpanel_t *p, const char *name, void *loc, int start_bit_number)
{
	...
	for (i = 0; i < num_names; i++) {
		if (bitnum <= 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindLight16: light %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
			      ...
			      uint16_t mask)
{
	...
	for (i = 0; i < num_names; i++) {
		if (bitnum <= 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindLight16invert: light %s bad bitnum %d\n", namelist[i],
				     bitnum);
			bitnum = 1;
		}
		...
		if (light) {
			...
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindLight16invert: light %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
			      ...
			      uint32_t mask)
{
	...
	for (i = 0; i < num_names; i++) {
		if (bitnum <= 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindLight32invert: light %s bad bitnum %d\n", namelist[i],
				     bitnum);
			bitnum = 1;
		}
		...
		if (light) {
			...
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindLight32invert: light %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
...
bool Lpanel_bindLight64(Lpanel_t *p, const char *name, void *loc, int start_bit_number)
{
	...
	for (i = 0; i < num_names; i++) {
		if (bitnum <= 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindLight64: light %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
			      ...
			      uint64_t mask)
{
	...
	for (i = 0; i < num_names; i++) {
		if (bitnum <= 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindLight64invert: light %s bad bitnum %d\n", namelist[i],
				     bitnum);
			bitnum = 1;
		}
		...
		if (light) {
			...
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindLight64invert: light %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
	...
	if (p->alpha_objects) {
		...
		for (i = 0; i < p->num_alpha_objects; i++) {
			if (p->alpha_objects[i]->texture_num) {
				lpTextures_bindTexture(&p->textures, p->alpha_objects[i]->texture_num);
			}
...
int Lpanel_pick(Lpanel_t *p, int button, int state, int x, int y)
{
	...
	if (num_picked) {
		...
		// SDL_Log("pick: switch_num=%d dir=%d\n", switch_num, switch_dir);
...
bool Lpanel_readConfig(Lpanel_t *p, const char *_fname)
{
	...
	if ((fd = fopen(fname, "r")) == 0) {
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
			     "readFile: could not open file %s\n", fname);
		free(fname);
		return 0;
	}
	...
	while (!feof(fd) && !bailout) {
		...
		if (!strcmp(token, "color")) {	// color
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("color defined outside of an object\n");
				bailout = true;
			} else {
				...
				if (n < 3) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("color must have 3 (r g b) values in the "
						"range of 0.0 - 1.0.\n");
					bailout = true;
				}
			}
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("envmap defined outside of an object\n");
				bailout = true;
			} else {
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("instance defined outside of an object\n");
				bailout = true;
			} else {
				if (!gtoken(buffer, s, 100, &pos)) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("instance with no object name.\n");
					bailout = true;
				} else
			...
			if (n < 3) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("lightcolor must have 3 (r g b) values in the "
					"range of 0.0 - 1.0.\n");
				bailout = true;
			}
			...
			if (n < 1) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("lightsize must have 1, 2, or 3 values (x y z).\n");
				bailout = true;
			}
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("line defined outside of an object\n");
				bailout = true;
			} else {
				if (!(p->curr_element = lpObject_addElement(p->curr_object,
									    p->curr_object))) {
					SDL_Log("could not allocate memory for line.\n");
					bailout = true;
				} else {
			...
			if (p->curr_object) {	// material reference
				...
				if (n != 1) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("material within an object definition must have "
						"one integer value.\n");
					bailout = true;
				}
				...
				if (n < 18) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("material definition must have 18 values.\n");
					bailout = true;
				} else
					lp_set_material_params(matnum, Ar, Ag, Ab, Aa, Dr, Dg, Db, Da,
							       Sr, Sg, Sb, Sa, shine, Er, Eg, Eb, Ea);
			}
			...
			if (!p->curr_element || !p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("normal defined outside of a polygon or line\n");
				bailout = true;
			} else {
				if (!p->curr_vertex) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("normal defined before a vertex definition\n");
					bailout = true;
				}
				...
				if (n < 3) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("normal must have 3 values.\n");
					bailout = true;
				} else
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("polygon defined outside of an object\n");
				bailout = true;
			} else {
				if (!(p->curr_element = lpObject_addElement(p->curr_object,
									    p->curr_object))) {
					SDL_Log("could not allocate memory for polygon.\n");
					bailout = true;
				} else {
			...
			if (!p->curr_object) {
				...
				if (n < 2) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("scene rotate must have 2 values.\n");
					bailout = true;
				}
			} else {
				n = sscanf(&buffer[pos], "%f %f %f",
				...
				if (n < 3) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("object rotate must have 3 values.\n");
					bailout = true;
				}
			}
			...
			if (!p->curr_element || !p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("texture coord defined outside of a polygon or line\n");
				bailout = true;
				break;
			} else {
				if (!p->curr_vertex) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("texture coord defined before a vertex coordinate "
						"was defined.\n");
					bailout = true;
					break;
				}
				...
				if (n < 2) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("texture coordinate must have 2 coordinates "
						"(e.g. t <s> <t>).\n");
					bailout = true;
					break;
				}
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("texture defined outside of an object.\n");
				bailout = true;
				break;
			}
			...
			if (!gtoken(buffer, token, TOKENSIZE, &pos)) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("texture with no path to jpeg file defined.\n");
				bailout = true;
				break;
			}
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("texture_scale defined outside of an object\n");
				bailout = true;
			} else {
				n = sscanf(&buffer[pos], "%f %f",
				...
				if (n < 2) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("texture_scale must have 2 (x y) values.\n");
					bailout = true;
				}
			}
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("texture_translate defined outside of an object\n");
				bailout = true;
			} else {
				n = sscanf(&buffer[pos], "%f %f",
				...
				if (n < 2) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("texture_translate must have 2 (x y) values.\n");
					bailout = true;
				}
			}
			...
			if (!p->curr_object) {
				...
				if (n < 3) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("scene translate must have 3 values.\n");
					bailout = true;
				}
			}
			...
			if (!p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("tristrip defined outside of an object\n");
				bailout = true;
			} else {
				if (!(p->curr_element = lpObject_addElement(p->curr_object,
									    p->curr_object))) {
					SDL_Log("could not allocate memory for tristrip.\n");
					bailout = true;
				} else {
			...
			if (!p->curr_element || !p->curr_object) {
				SDL_Log("Error on line %d of config file %s\n", lineno, fname);
				SDL_Log("vertex defined outside of a polygon or line\n");
				bailout = true;
			} else {
				if (!(p->curr_vertex = lpElement_addVertex(p->curr_element))) {
					SDL_Log("could not allocate memory for vertex.\n");
					bailout = true;
				}
				...
				if (n < 2) {
					SDL_Log("Error on line %d of config file %s\n", lineno, fname);
					SDL_Log("vertex must have 2 or 3 coordinates.\n");
					bailout = true;
				}
			}
		} else {
			SDL_Log("Error: unknown statement on line %d of config file %s\n", lineno,
				fname);
			bailout = true;
...
void Lpanel_resolveObjectInstances(Lpanel_t *p)
{
	...
	for (i = 0; i < p->num_objects; i++) {
		if (p->objects[i]->instance_name) {
			if ((obj = Lpanel_findObjectByName(p, p->objects[i]->instance_name)))
				...
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "Error: object %s instances object %s which "
					     "cannot be found.\n",
	...
	for (i = 0; i < p->num_objects; i++) {
		...
		while (obj) {
			for (j = 0; j < 3; j++) {
				p->objects[i]->bbox.xyz_min[j] = min(p->objects[i]->bbox.xyz_min[j],
								     obj->bbox.xyz_min[j]);
				p->objects[i]->bbox.xyz_max[j] = max(p->objects[i]->bbox.xyz_max[j],
								     obj->bbox.xyz_max[j]);
			}
...
void Lpanel_sampleData(Lpanel_t *p)
{
	...
	if (*p->simclock < p->old_clock) {
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
			     "libfrontpanel: Warning clock went backwards (current=%" PRIu64
...
void Lpanel_sampleLightGroup(Lpanel_t *p, int groupnum, int clockval)
{
	...
	if (groupnum < 0 || groupnum >= LP_MAX_LIGHT_GROUPS) {
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
			     "sampleLightGroup: groupnum (%d) must be in the range of (0-%d). \n",
...
void lpLight_draw(lpLight_t *p)
{
	...
	case LBINDTYPE_FLOATV:
		...
		// SDL_Log("xyzzy: intense=%f bitnum=%d\n", p->intensity, p->bitnum);
		break;
	}
...
{
	SDL_Log("light: name=%s\n", p->name);

	SDL_Log("obj: pos=%f %f %f size=%f %f %f color=%f %f %f\n",
		p->parms->pos[0],
		p->parms->pos[1],
		p->parms->pos[2],
		p->parms->scale[0],
...
#if 0
	// SDL_Log("calcIntensity runflag = %d\n", *p->runflag);
	if (*p->runflag)
		// if (!strcmp(p->name, "LED_ADDR_15") && p->intensity > .2)
		if (!strcmp(p->name, "LED_ADDR_15")) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "dbg: draw: %s intense=%f on_time=%d old_clock=%d "
...
void lpLight_setupData(lpLight_t *p)
{
	if (p->obj_refname) {
		if (!(p->obj_ref = Lpanel_findObjectByName(p->panel, p->obj_refname))) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "error: light %s references object %s which cannot be found.\n",
...
bool Lpanel_smoothLight(Lpanel_t *p, const char *name, int nframes)
{
	...
	if (nframes < 0) {
		SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
			     "smoothLight: light %s error. nframes = %d must be >=0 \n",
			     name, nframes);
		return false;
	}
