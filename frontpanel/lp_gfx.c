...
void Lpanel_growAlphaObjects(Lpanel_t *p)
{
	...
	new_alpha_objects = (lpObject_t **) realloc(p->alpha_objects,
						    sizeof(lpObject_t *) * (p->num_alpha_objects + 1));
...
void lpElement_genTextureCoords(lpElement_t *p, lpObject_t *obj, lpBBox_t *bbox)
{
	...
	// vp->st[j] = (1.0 / (bbox->xyz_max[j] - bbox->xyz_min[j])) * (vp->xyz[j] - bbox->xyz_min[j]);
		...
		if (tp->surface) {
			...
			default:
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "addTexture: Invalid # texture components.\n");
				return 0;
			}
			...
			if (SDL_MUSTLOCK(tp->surface) && SDL_LockSurface(tp->surface) < 0) {
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "addTexture: Can't lock SDL surface.\n");
				return 0;
			}
			...
			tp->texels = (unsigned char *) malloc(tp->texSsize * tp->texTsize * tp->imgZsize);
...
void lpTextures_bindTexture(lpTextures_t *p, int n)
{
	if (n == 0)
		...
		if (n > p->num_textures) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindTexture: Invalid texture number (%d)\n", n);
			return;
		} else {
			// SDL_Log("bind %d %d\n", n, tex[n]->bind_id);
