...
void lpSwitch_setupData(lpSwitch_t *p, int sw_num)
{
	...
	for (i = 0; i < p->num_object_refs; i++) {
		if (!(p->object_refs[i] = Lpanel_findObjectByName(p->panel, p->object_ref_names[i]))) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "error: switch %s references object %s which cannot be found.\n",
	...
	case LP_SWITCH_GFX_TOGGLE:
		SDL_Log("setupData: toggle\n");
		break;

	case LP_SWITCH_GFX_PADDLE:
		SDL_Log("setupData: paddle\n");
		break;
	...
	case LP_SWITCH_GFX_OBJECT_REF:
		...
		break;
	default:
		SDL_Log("setupData: invalid switch type %d\n", p->type);
		break;
	}
	...
	while ((n = Parser_parse(&parser, &result)) < 0) {
		if (n != PARSER_DONE) {
			...
			if (!strcmp(switch_parse_rules[result->cmd_idx].cmd, "objects")) {
				sw->object_ref_names = (char **) malloc(sizeof(char *) * result->num_args);
				...
				if (!strcmp(result->strings[0], "toggle")) {
					...
					SDL_Log("Invalid switch 'operation'. Should be one "
				...
				if (!strcmp(result->strings[0], "toggle")) {
					...
					SDL_Log("Invalid switch 'type'. Should be one "
	...
	for (i = 0; i < num_names; i++) {
		...
		if (sw) {
			...
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "addSwitchCallback: switch %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
...
bool Lpanel_bindSwitch8(Lpanel_t *p, const char *name, void *loc_down, void *loc_up, int start_bit_number)
{
	char **namelist;
	int num_names;
	int i;
	...
	for (i = 0; i < num_names; i++) {
		// SDL_Log("name %d = %s bitnum=%d\n", i, namelist[i], bitnum);

		if (bitnum < 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindSwitch8: switch %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
		...
		if (sw) {
			lpSwitch_bindData8(sw, (uint8_t *) loc_down, (uint8_t *) loc_up, bitnum - 1);
		} else {
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindSwitch8: switch %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
...
bool Lpanel_bindSwitch16(Lpanel_t *p, const char *name, void *loc_down, void *loc_up, int start_bit_number)
{
	char **namelist;
	int num_names;
	int i;
	...
	for (i = 0; i < num_names; i++) {
		...
		if (bitnum < 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindSwitch16: switch %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
		...
		if (sw) {
			lpSwitch_bindData16(sw, (uint16_t *) loc_down, (uint16_t *) loc_up, bitnum - 1);
		} else {
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindSwitch16: switch %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
...
bool Lpanel_bindSwitch32(Lpanel_t *p, const char *name, void *loc_down, void *loc_up, int start_bit_number)
{
	char **namelist;
	int num_names;
	int i;
	...
	for (i = 0; i < num_names; i++) {
		...
		if (bitnum < 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindSwitch32: switch %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
		...
		if (sw) {
			lpSwitch_bindData32(sw, (uint32_t *) loc_down, (uint32_t *) loc_up, bitnum - 1);
		} else {
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindSwitch32: switch %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
...
bool Lpanel_bindSwitch64(Lpanel_t *p, const char *name, void *loc_down, void *loc_up, int start_bit_number)
{
	char **namelist;
	int num_names;
	int i;
	...
	for (i = 0; i < num_names; i++) {
		...
		if (bitnum < 0) {
			SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
				     "bindSwitch64: switch %s bad bitnum %d\n", namelist[i], bitnum);
			bitnum = 1;
		}
		...
		if (sw) {
			lpSwitch_bindData64(sw, (uint64_t *) loc_down, (uint64_t *) loc_up, bitnum - 1);
		} else {
			if (!p->ignore_bind_errors)
				SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
					     "bindSwitch64: switch %s not found\n", namelist[i]);
			status = false;
		}
		if (namelist[i])
			free(namelist[i]);
